...
#ifdef MXARC
#define main_mxarc main
#define mxlexit_mxarc mxlexit
#define mxlargs_mxload mxlargs
#endif

/*
 * Read Multics archives and retrieve
 * components. This is basically a
...

{
  ...
  for (i = 0; i < n_archives; ++i)
  {
    ...
    {
      fprintf(stderr, "Cannot open Multics archive file %s.\n", argv[i]);
      continue;
    }

        /*
         * Read what should be header of
         * first component to see if we have
         * an already 8bit-ified archive, a raw
         * 9bit archive, or no archive at all.
         * For MS-DOS, the translation of NL to
         * CR-LF means that we cannot reliably
         * read an 8bit-ified archive.
         */
...
#define BSD
      ...
      if (rename_result != 0)
      {
        ...
        continue;
          ...
          /*
           * Put results in directory
           * with old archive name
           */

      strcpy(retrieval_options_ptr->new_path, argv[i]);
    }
    dearc_result = mxdearc(
      archive_file,
      &preamble,
      retrieval_options_ptr,
      &branch_preamble,
...
#endif
{
  ...
  fprintf(stderr,
    "-n\t\t= no map\n");
  fprintf(stderr,
    "-t\t\t= table; just produce a map\n");
  fprintf(stderr,
    "-u\t\t= unpack; replace archive by ");
  fprintf(stderr,
...

{
 ...
 /*
  * Check conversion type of segment and
  * call convert_seg to do the appropriate
  * conversion, except for "discard".
  *
  * The conversion type is determined
  * in one of two ways:
  *
  *      1) Explicitly specified in the control file
  *         with a force_convert statement. In this
  *         case, it doesn't matter what the segment
  *         type is.
  *
  *      2) Determined from the segment type, which
  *         can itself be determined in two ways:
  *
  *              a) From the segment's name, not requiring
  *                 that the segment be read first.
  *
  *              b) From the segment's contents, in which case
  *                 the segment must be read first, and may
  *                 then not be used if the conversion type
  *                 turns out to be "discard".
  */
...
#else
void
process_seg_mxarc(infile, branch_preamble_ptr, preamble_ptr,
...
EXIT:
  ...
  release_temp_file(contents_file,
    "raw file contents");
}

 /*
  * This function can be called by mxdearc
  * in place of the usual process_seg
  * function, to process preconverted-to-8bit
  * segments.  It must only be called for
  * segment types "ascii" and "ascii_archive"
  * (for the *all important* recursive archive
  * unpacking)  when the conversion type for
  * "ascii" will be "8bit" and for "ascii_archive"
  * it will be "unpack".
  */
...
#else
void
process_ascii_seg_mxarc(branch_preamble_ptr, contents_file, preamble_ptr,
                       retrieval_options_ptr)
...
#else
static int
find_and_mark_in_list(list, n_entries, entry)
...

{
           ...
           && strcmp(seg_type, "ascii_archive") == 0)
  {
        ...
        /*
         * Normal case for unpacking an
         * ascii archive will be to pass
         * mxdearc the preconverted-to-8bit
         * segment.  Unpacked components
         * will be passed to process_ascii_seg.
         * However, in the unlikely event
         * that the conversion to be done for
         * subsquently unpacked ascii segments
         * is other than 8bit, we must go the
         * slow route and use the original 9bit
         * segment so that we can get 9bit
  ...
  else
  {
    fprintf(
      stderr,
      "Conversion type %s is not yet implemented ",
      conversion_type);
