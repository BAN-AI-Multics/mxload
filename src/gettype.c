...
#ifdef ANSI_FUNC
char *
get_type_by_name (struct PREAMBLE *preamble_ptr)
#else
char
*get_type_by_name(preamble_ptr)
...
/*
 * Read the file contents to determine its type.
 * "ascii", "nonascii" and "object" are the only
 * possible types returned.  We actually go ahead
 * here and "preconvert" the segment to 8-bit ASCII.
 * If the segment turns out to be ascii the whole
 * segment will have been read.  Re-reading it to then
 * do the conversion is expensive.  If the segment
 * turns out not to be ascii, chances are that only a
...
#ifdef ANSI_FUNC
char *
get_type_by_content (MXBITFILE *contents_file, long bitcount,
                MXBITFILE *preconverted_contents_file)
#else
char
*get_type_by_content(contents_file, bitcount, preconverted_contents_file)
...

{
  ...
  int first_chunk;
#ifndef WORDALIGN
  static unsigned long ninth_bit_mask[2] = {
    0L, 0L
  };
  unsigned long *mask_overlay;
#else  /* ifndef WORDALIGN */
  static unsigned char ninth_bit_mask[8]
    = {
    '\0', '\0', '\0', '\0', '\0', '\0', '\0', '\0'
    };
  int byteno;
#endif /* ifndef WORDALIGN */

  rewind_mxbit_file(contents_file, "rt");
  rewind_mxbit_file(preconverted_contents_file, "wt");

  /*
   * Initialize mask for high-order bit
   * checking.  Done at run time to
   * work on byte-reversed and
   * non-byte-reversed machines
   */
  ...
  while (n_72_bit_chunks_left > 0)
  {
        ...
        /*
         * Try to read the biggest possible chunk,
         * limited by remaining space in tape block
         * if reading from tape, EXCEPT that the
         * first chunk should be small in case the
         * segment quickly turns out to be non-ascii.
         */
  ...
  /*
   * And read to the end of the file using bit
   * i/o routines to pick remaining bytes apart
   */
...
NONASCII:
 ...
 /*
  * Determine if file is
  * Multics object segment
  */
...

{
  ...
  /*
   * First word should contain
   * a 2 (the version number)
   */
