...

{
  ...
  if (preamble_ptr->record_type == SEGMENT_RECORD)
  {
    fprintf(
      mxlargs.map_file,
      "%3d",
      (int)( preamble_ptr->length + 1023 ) / 1024);
    fprintf(mxlargs.map_file, " %s", "SEG");
  }
  else
  {
    fprintf(mxlargs.map_file, "    %s", "DIR");
  }
  ...
  fprintf(
    mxlargs.map_file,
    "   %s",
    timestr(&( branch_preamble_ptr->dtu )));
  fprintf(mxlargs.map_file, "\n");
  ...
  {
    fprintf(
      mxlargs.map_file, "%37sDTBM:            %s\n",
          "",
      timestr(&( branch_preamble_ptr->dtbm )));
    fprintf(
          mxlargs.map_file, "%37sDTD:             %s\n",
          "",
      timestr(&( branch_preamble_ptr->dtd )));
    fprintf(
          mxlargs.map_file, "%37sUID:             ",
          "");
    fprintf(
          mxlargs.map_file, "%.6lo",
          branch_preamble_ptr->uid[0]);
    fprintf(
          mxlargs.map_file, "%.6lo\n",
          branch_preamble_ptr->uid[1]);
    fprintf(
      mxlargs.map_file, "%37sRing Brackets:   ",
          "");
    for (i = 0; i < 3; ++i)
    {
      fprintf(mxlargs.map_file, "%d ", branch_preamble_ptr->rings[i]);
    }
    ...
    {
      format_access_class(
        branch_preamble_ptr->access_class,
        access_class_string);
      fprintf(
        mxlargs.map_file,
        "%37sAccess Class:    %s",
...

{
  ...
  for (i = 0; i < dirlist_preamble_ptr->nlinks; ++i)
  {
    ...
    fprintf(
          mxlargs.map_file, " %-32s",
          link_ptr->ename);
    fprintf(
      mxlargs.map_file, "   ");
    fprintf(
          mxlargs.map_file, " LINK");
    fprintf(
          mxlargs.map_file, "  %s",
          timestr(&( link_ptr->dtm )));
    fprintf(
          mxlargs.map_file, "\n");
    if (mxlargs.verbose || mxlargs.extremely_verbose)
    {
...
#else
void
format_access_class(access_class, access_class_string)
...

{
  ...
  /*
   * If any supposedly unused
   * bits are non-zero, redisplay
   * the whole thing in octal
   */
...
#else
void
display_conversion_info(newpath, segtype, conversion_type)
...

{
  ...
  {
    fprintf(
      mxlargs.map_file, "%19sSegment Type:    %s\n", "",
          segtype);
  }
  ...
  fprintf(
    mxlargs.map_file, "%19sLoading Into:    %s\n", "",
    newpath);
}
