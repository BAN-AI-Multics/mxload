 ...
 *    and Oxford Systems, Inc.    *
 ...
 /*
  * Routine to read an mxload control file
  * and produce a chained list of structures
  * describing the files to be loaded and
  * the options that apply to them.
  * Any errors are reported by writing to
  * stderr and execution is aborted by calling
  * mxlexit. No claims of prettiness are made
  * for this program.  It would be more properly
  * done using yacc, but is done in straight
  * C for portability.
  */
...
#else
struct MXLOPTS *
parsctl(n_ctl_paths, ctl_paths, n_subtree_paths,
                        subtree_paths, return_defaults)
...

{
  ...
  while (1)
  {
    ...
    if (line == NULL && ctl_pathN < n_ctl_paths)
    {
      ...
      {
        ctl_file = fopen(ctl_paths[ctl_pathN], "r");
      }
    ...
    if (strcmp(
          stmt_type,
          "file") == 0 || strcmp(stmt_type, "directory") == 0
        || strcmp(stmt_type, "subtree") == 0)
    {
      if (first_opt_struct == NULL)
      {
        last_opt_struct = first_opt_struct
                            = (struct MXLOPTS *)malloc(
                                sizeof ( struct
                                         MXLOPTS ));
      }
      else
      {
        last_opt_struct = last_opt_struct->next
                            = (struct MXLOPTS *)malloc(
                                sizeof ( struct
                                         MXLOPTS ));
      }
      ...
      path_stmts(token_ptr, &default_options, stmt_type, last_opt_struct);
    }
    else if (strcmp(lowercase_stmt_type, "convert") == 0)
    {
      convert_stmt(
        token_ptr,
        isupper(stmt_type[0]) ? &default_options
                                                      : last_opt_struct);
    }
    ...
    else if (strcmp(lowercase_stmt_type, "person") == 0
             || strcmp(lowercase_stmt_type, "project") == 0)
    {
      user_stmt(
        token_ptr,
        isupper(stmt_type[0]) ? &default_options : last_opt_struct,
    ...
    else if (strcmp(lowercase_stmt_type, "force_convert") == 0)
    {
      force_convert_stmt(
        token_ptr,
        isupper(
          stmt_type[0]) ? &default_options
                                                         : last_opt_struct);
    }
    else if (( attr_index
                 = get_keyword_values_index(
                     attr_cv_types,
                     lowercase_stmt_type))
             >= 0)
    {
  ...
  /*
   * Break out when at end
   * of last control file
   */
  ...
  else
  {
    ...
    while (n_subtree_paths > 0)
    {
      ...
      memset(last_opt_struct, 0, sizeof ( struct MXLOPTS ));
      fake_subtree_and_new_path_stmts(
        &default_options,
        last_opt_struct,
        subtree_paths[0],
        subtree_paths[1]);
    ...
    if (first_opt_struct == NULL)
    {
      ...
      fprintf(stderr,
            "contain at least one file, directory or\n");
      fprintf(stderr,
            "subtree statement.\n");
      ++error_count;
    }
...
#else
static void
path_stmts(headptr, default_options_ptr, stmt_type,
                       file_opt_ptr)
...
#else
static void
fake_subtree_and_new_path_stmts(default_options_ptr,
                                            file_opt_ptr,
                                            multics_path, unix_path)
...
#else
static void
copy_options_struct(to_options_ptr, from_options_ptr)
...

{
  ...
  {
    file_opt_xlation_ptr = \
      (struct USER_TRANSLATION *)malloc(sizeof ( struct USER_TRANSLATION ));
    memcpy(
      (char *)file_opt_xlation_ptr,
    ...
    {
      prev_file_opt_xlation_ptr->next = file_opt_xlation_ptr;
    }
...

{
          ...
          get_keyword_values_index(file_cv_types, segment_type)) < 0)
  {
    ...
    fprintf(
      stderr,
      "Invalid file type %s in convert statement.\n",
      segment_type);
    display_statement();
    return;
  }
      ...
      == NULL)
  {
    ...
    fprintf(
      stderr,
      "Invalid conversion type for %s file type:  %s\n",
      segment_type,
      conversion_type);
    display_statement();
  }
...

{
  ...
  if (( optptr->list_values[list_idx]
          = get_keyword_value(list_types[list_idx], list_type))
      == NULL)
  {
    display_line_no();
...
#else
static void
attr_stmt(stmt_type, attr_idx, headptr, optptr)
...

{
  ...
  if (( optptr->attr_cv_values[attr_idx]
          = get_keyword_value(attr_cv_types[attr_idx], conversion_type))
      == NULL)
  {
    display_line_no();
    fprintf(
      stderr,
      "Invalid conversion type in %s statement:  %s\n",
      stmt_type,
      conversion_type);
    display_statement();
  }
...
#else
static void
force_convert_stmt(headptr, optptr)
...

{
  ...
  copy_token_str(
    force_convert_type,
    sizeof ( force_convert_type ),
    headptr->next);
  if (( optptr->force_convert
          = get_keyword_value(
              force_convert_file_cv_types,
              force_convert_type))
      == NULL)
  {
    display_line_no();
    fprintf(
      stderr,
      "Invalid conversion type in force_convert statement:  %s\n",
      force_convert_type);
    display_statement();
  }
}
...

{
  ...
  else
  {
    ...
    fprintf(
      stderr,
      "Invalid reload type in reload statement:  %s\n",
      reload_type);
    display_statement();
  }
}
...

{
  ...
  else
  {
    ...
    fprintf(
      stderr,
      "Invalid access type in access statement:  %s\n",
      access_type);
    display_statement();
  }
}
...

{
  ...
  else
  {
    ...
    fprintf(
      stderr,
      "Invalid dataend type in dataend statement:  %s\n",
      dataend_type);
    display_statement();
  }
}
...
#else
static void
user_stmt(headptr, optptr, stmt_type)
...

{
  ...
  new_user_translation
    = (struct USER_TRANSLATION *)malloc(sizeof ( struct USER_TRANSLATION ));
  copy_token_str(
    new_user_translation->multics_name,
    sizeof ( new_user_translation->multics_name ),
    multics_name_token_ptr);
  ...
  if (strcmp(stmt_type, "person") == 0)
  {
    ...
    if (new_user_translation->id == -1)
    {
      if (strcmp(new_user_translation->unix_name, "(process)") != 0)
      {
        ...
        {
          new_user_translation->id = pwp->pw_uid;
        }
    ...
    else
    {
      ...
      if (pwp == NULL)
      {
        ...
        fprintf(
          stderr,
          "UID %d not found in password file.\n",
          new_user_translation->id);
        display_statement();
      }
      else
      {
        strcpy(new_user_translation->unix_name, pwp->pw_name);
      }
  ...
  else /* "project" statement */
  {
    ...
    if (new_user_translation->id == -1)
    {
      if (strcmp(new_user_translation->unix_name, "(process)") != 0)
      {
        ...
        {
          new_user_translation->id = grp->gr_gid;
        }
    ...
    else
    {
      ...
      {
        strcpy(new_user_translation->unix_name, grp->gr_name);
      }
...

{
  ...
  /*
   * White space in front of statement
...
#ifdef ANSI_FUNC
static struct token *
make_token (char *strptr, int strlen, int is_delim, struct token *headptr)
#else
static struct token *
make_token(strptr, strlen, is_delim, headptr)
...
/*
 * Function to search through a keyword_array to
 * find the keyword_values with the specified
 * keyword, returning a pointer to the keyword_values.
 */

#ifdef ANSI_FUNC
char **
get_keyword_values (char **keyword_array[], char *keyword)
#else
char **
get_keyword_values(keyword_array, keyword)
...
#endif
{
 ...
 /*
  * Function to search through a
  * keyword_array to find the number
  * of keyword_values
  */
...

{
  ...
  * keyword_values in the keyword_array.
  */
...
#else
int
get_keyword_values_index(keyword_array, keyword)
...
#endif
{
 ...
 /*
  * Function to search through a keyword_values
  * to find the specified value and return a
  * pointer to it, or NULL if it is not found.
  */

#ifdef ANSI_FUNC
char *
get_keyword_value (char *keyword_values[], char *value)
#else
char *
get_keyword_value(keyword_values, value)
...
#else
static void
copy_token_str(target_str, target_str_len, token_ptr)
...
#else
static int
check_tokens(headptr, n_tokens_required)
 ...
 * Ignores whitespace before and after
 * statements, as well as comments.
 * Returns 0 if line successfully
 * read, non-0 at eof. Aborts by calling
 * mxlexit in case of read error,
 * or file ending in comment.
 */

#ifdef ANSI_FUNC
static char *
get_next_stmt (FILE *ctl_file)
#else
static char *
get_next_stmt(ctl_file)
...

{
  ...
  while (1)
  {
    ...
    if (c == '/')
    {
      ...
      if (next_c == '*')
      {
        while (c != '*' || next_c != '/')
        {
          ...
          {
            display_line_no();
            fprintf(stderr, "Control file ends in a comment.\n");
            display_statement();
          }
        }
        c = getc_counting_lines(ctl_file);
      ...
      {
        ungetc_counting_lines(ctl_file, next_c);
      }
    }
    ...
    else if (c == EOF)
    {
      ...
      {
        display_line_no();
        fprintf(stderr, "Last statement does not end with a semicolon.\n");
        display_statement();
      }
...
#endif
{
  fprintf(
    stderr,
    "Error on line %d of control file %s.\n",
    cur_line_no,
    cur_ctl_path);
  ++error_count;
}
...
#ifdef ANSI_FUNC
static struct MXLOPTS *
resort_options (struct MXLOPTS *head)
#else
static struct MXLOPTS *
resort_options(head)
...
#ifdef ANSI_FUNC
static struct MXLOPTS *
remove_from_list (struct MXLOPTS *head_ptr, struct MXLOPTS *opt_ptr)
#else
static struct MXLOPTS *
remove_from_list(head_ptr, opt_ptr)
...
#ifdef ANSI_FUNC
static struct MXLOPTS *
add_to_list (struct MXLOPTS *head_ptr, struct MXLOPTS *opt_ptr)
#else
static struct MXLOPTS *
add_to_list(head_ptr, opt_ptr)
