...
#ifdef MXLOAD
#define main_mxload main
#define mxlexit_mxload mxlexit
#define mxlargs_mxload mxlargs
#endif
#ifdef MXARC
#define mxlargs_mxload mxlargs
#endif
...
#ifdef MXMBX
#define mxlexit_mxload mxlexit
#define process_seg process_seg_mxmbx
#endif

 /*
  * Read Multics backup file or
  * tape and retrieve segments.
  */
...
int write_quoted_string (FILE *file, char *string);
...

{
  ...
  memset(&mxlargs_mxload, 0, sizeof ( mxlargs_mxload ));
  mxlargs_mxload.map_file = NULL;
  mxlargs_mxload.map_filename = NULL;
  while (( option_letter = getopt(argc, argv, "bc:g:lnvx")) != EOF)
  {
    switch (option_letter)
    {
    case 'b': /* Brief: suppress no-match warnings */
      mxlargs_mxload.brief = 1;
      break;
    ...
    case 'g': /* Global map file */
      mxlargs_mxload.map_filename = \
        strcpy(malloc(strlen(optarg) + 1), optarg);
      mxlargs_mxload.local_map = 0; /* Override -l */
      mxlargs_mxload.no_map = 0;    /* Override -n */
      break;

    case 'l': /* Local map files:
               * mxload.map in each directory */
      mxlargs_mxload.local_map = 1;
      mxlargs_mxload.no_map = 0;          /* Override -n */
      mxlargs_mxload.map_filename = NULL; /* Override -g */
      break;

    case 'n': /* No map */
      mxlargs_mxload.no_map = 1;
      break;

    case 'v': /* Verbose:  long listing output */
      mxlargs_mxload.verbose = 1;
      break;

    case 'x': /* Extremely verbose:
               * full listing output */
      mxlargs_mxload.extremely_verbose = 1;
      break;
  ...
  if (mxlargs_mxload.map_filename != NULL)
  {
    mxlargs_mxload.map_file = fopen(mxlargs_mxload.map_filename, "a");
  }
  else if (mxlargs_mxload.no_map)
  {
    mxlargs_mxload.map_file = fopen("/dev/null", "w");
  }
  else if (!mxlargs_mxload.local_map)
  {
    mxlargs_mxload.map_file = stdout;
  }
  ...
  while (rdbkrcd(mx_backup_file, preamble_file, &preamble) == 0)
  {
    if (preamble.record_type == SEGMENT_RECORD
        && ( retrieval_options_ptr
               = get_options_ptr(&preamble, retrieval_list_ptr))
        != NULL)
    {
    ...
    {
      if (strcmp(
            retrieval_options_ptr->list_values[get_keyword_values_index(
                                                 list_types,
                                                 "link")],
            "none")
          != 0)
...
#else
static void
file_links(preamble_ptr, dirlist_preamble_ptr,
...

{
  ...
  /*
   * Make pathname for file mxload.links
   * in new directory (if local listing)
   * or working directory if global listing
   */
  ...
  for (i = 0; i < dirlist_preamble_ptr->nlinks; ++i)
  {
    ...
    if (global_file || retrieval_options_ptr->reload[0] == 'f')
    {
      write_quoted_string(
        links_file,
        strcat(
          strcat(strcpy(temp_str, preamble_ptr->dname), ">"),
          link_ptr->ename));
    }
    else
    {
      write_quoted_string(links_file, link_ptr->ename);
    }
...
#endif
{
  ...
  fprintf(stderr,
        "-l\t\t= local map files; ");
  fprintf(stderr,
        "direct map to files in target directories\n");
  fprintf(stderr,
        "-n\t\t= no map\n");
  fprintf(stderr,
        "-v\t\t= verbose; produce long map\n");
  fprintf(stderr,
        "-x\t\t= extremely verbose; produce full map\n");
  mxlexit(4);
}
...

{
 ...
 /*
  * Check conversion type of segment & call convert_seg
  * to do the appropriate conversion, except for "discard".
  *
  ...
  *                 be read first.
  *
  *              b) From the segment's contents, in which case the segment
  *                 must be read first, and may then not be used if the
  *                 conversion type turns out to be "discard".
  */
...
#else
void
process_seg(infile, branch_preamble_ptr, preamble_ptr,
...

{
  ...
  else
  {
    ...
    else
    {
          ...
          || strcmp(seg_type_by_name, "archive") == 0)
      {
        ...
        seg_has_been_read = 1;
        seg_type_by_content = get_type_by_content(
          contents_file,
            ...
            && strcmp(seg_type_by_name, "archive") == 0)
        {
          ...
          {
            seg_type = "nonascii_archive";
          }
        }
        else
        {
          seg_type = seg_type_by_content;
        }
      }
      else
      {
        seg_type = seg_type_by_name;
      }
    }

    conversion_type
      = retrieval_options_ptr->file_cv_values[get_keyword_values_index(
                                                file_cv_types,
                                                seg_type)];
  }
  ...
  if (!seg_has_been_read && rdseg(infile, contents_file, preamble_ptr) != 0)
  {
    goto EXIT;
  }
...
EXIT:
 ...
 /*
  * This function can be called by mxdearc in
  * place of the usual process_seg function, to
  * process preconverted-to-8bit segments.  It must
  * only be called for segment types "ascii" and
  * "ascii_archive" (for the *all important* recursive
  * archive unpacking)  when the conversion type for "ascii"
  * will be "8bit" and for "ascii_archive" it will be "unpack".
  */
...
#else
void
process_ascii_seg(branch_preamble_ptr, contents_file, preamble_ptr,
                       retrieval_options_ptr)
...
#else
static int
make_new_path_fixed_name(dname, ename, retrieval_options_ptr,
                                    name_type, new_path)
...

{
      ...
      && strcmp(seg_type, "ascii_archive") == 0)
  {
         ...
         * event that the conversion to be done for subsquently unpacked
         * ascii segments is other than 8bit, we must go the slow route
         * and use the original 9bit segment so that we can get 9bit
         * conversion done on components.
         */
    ...
    {
      fprintf(stderr, "Archive will be stored using 8bit conversion.\n");
      conversion_type = "8bit";
    }
    else
    {
      ...
      && strcmp(seg_type, "nonascii_archive") == 0)
  {
    ...
    if (unpacker_status < 0)
    {
      fprintf(stderr, "Archive will be stored using 9bit conversion.\n");
      ...
      && ( strcmp(seg_type, "mbx") == 0 || strcmp(seg_type, "ms") == 0 ))
  {
    ...
    if (unpacker_status < 0)
    {
      fprintf(stderr, "Mailbox will be stored using 9bit conversion.\n");
      conversion_type = "9bit";
    }
    else
    {
  ...
  if (strcmp(
        conversion_type,
        "repack") == 0 && strcmp(seg_type, "mbx") == 0)
  {
    unpacker_status
      = mxmseg(
          contents_file,
...

{
  ...
  {
    get_project(branch_preamble_ptr->author, project_id);
  }
  ...
  {
    get_person(branch_preamble_ptr->author, person_id);
  }

  /*
   * Set permission bits according
   * to ACL if requested
   */
  ...
  if (retrieval_options_ptr->user_translations != NULL)
  {
    set_owner(
      new_path,
      retrieval_options_ptr->user_translations,
      project_id,
      person_id);
  }
...

{
  ...
  if (mxlargs_mxload.verbose || mxlargs_mxload.extremely_verbose)
  {
    fprintf(mxlargs_mxload.map_file,
...
#else
static void
set_owner(new_path, xlation_ptr, project_id, person_id)
...

{
  ...
  if (chown_result != 0)
  {
    ...
    fprintf(
      stderr,
      "Error setting owner/group to %s(%d)/%s(%d).\n",
      owner_name,
      uid,
      group_name,
      gid);
...

{
  ...
  {
    write_quoted_string(acl_file, preamble_ptr->ename);
  }
  ...
  {
    acl_ptr = branch_preamble_ptr->acl + i;
    fprintf(acl_file, " %s", SHORT_SEG_ACCESS_MODES[acl_ptr->mode[0] >> 5]);
    fprintf(acl_file, " ");
...

{
  ...
  /*
   * Make pathname for file mxload.addname
   * in new directory (if local directory or
   * subtree) or new_path directory
   */
  ...
  {
    write_quoted_string(addname_file, preamble_ptr->ename);
  }
...
#else
static void
open_local_map(dir, retrieval_options_ptr)
...

{
  ...
  {
    make_new_path_fixed_name(
      dir,
      "mxload.map",
      retrieval_options_ptr,
      name_type,
      local_map_path);
    if (mxlargs_mxload.map_file != NULL)
    {
      fclose(mxlargs_mxload.map_file);
    }

    mxlargs_mxload.map_file = fopen(local_map_path, "a");
    if (mxlargs_mxload.map_file == NULL)
    {
      mxlargs_mxload.map_file = stdout;
    }
